(ns statanalysis.actor
  "StatisticalAnalysisActor — the ISCO-08 3314 community statistical analysis
  actor as a `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one statistical analysis operation request
  ...
  The unconditional invariant: the AnalysisAdvisor can never
  directly publish a report or finalize an analysis the
  StatisticalAnalysisGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [statanalysis.advisor :as advisor]
            [statanalysis.governor :as governor]
            [statanalysis.store :as store]))

(defn build-graph
  "Build a compiled StatisticalAnalysisActor graph. `store` implements
  `statanalysis.store/Store`. `advisor` implements
  `statanalysis.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :dataset-id (:dataset-id proposal)
                                    :payload proposal}]
